Back_sim_fcn <- function(N.sim,center=c(0,0),scale=1,theta=0) {
#N.sim <- 20000
...
#plot(X.s[Miss.ID==0],Y.s[Miss.ID==0],pch='.')

X.s <- scale*X.s[Miss.ID==0]
Y.s <- scale*Y.s[Miss.ID==0]

R <- cbind(c(cos(theta),sin(theta)),c(-sin(theta),cos(theta)))

# Calculate the(X,Y) coordinates rotated by -t_hat.
X.new <- cbind(X.s,Y.s)%*%t(R)[,1]
Y.new <- cbind(X.s,Y.s)%*%t(R)[,2]

X.new <- X.new + center[1]
Y.new <- Y.new + center[2]

return(cbind(X.new,Y.new))
